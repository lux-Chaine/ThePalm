...
<section class="events" id="meetings">
  <div class="events-grid">
    <div class="events-gallery" use:reveal>
    ...
    <div class="events-copy" use:reveal={{ delay: 200 }}>
      <span class="eyebrow">{$t.events.eyebrow}</span>
      <h3>{$t.events.title}</h3>
      <p>{$t.events.description}</p>

      <div class="capacity-grid">
        {#each $t.events.capacity as cap}
          <div><strong>{cap.value}</strong><span>{cap.label}</span></div>
        {/each}
      </div>

      <div class="max-cap">
        <div><span>{$t.events.maxCap.capacity.label}</span><strong>{$t.events.maxCap.capacity.value}</strong></div>
        <div><span>{$t.events.maxCap.availability.label}</span><strong>{$t.events.maxCap.availability.value}</strong></div>
      </div>

      <div class="venue-table">
        <div class="venue-row venue-head">
          <span>{$t.events.venueTable.head.venue}</span>
          <span>{$t.events.venueTable.head.area}</span>
          <span>{$t.events.venueTable.head.capacity}</span>
        </div>
        {#each $t.events.venueTable.rows as row}
          <div class="venue-row">
            <span>{row.name}</span><span>{row.area}</span><span>{row.capacity}</span>
          </div>
        {/each}
      </div>
    </div>
...
<style>
  ...
  .eyebrow { font-family: 'Work Sans', sans-serif; font-size: 0.72rem; letter-spacing: 0.22em; text-transform: uppercase; font-weight: 600; color: var(--gold-light); display: block; margin-bottom: 18px; }
  .events-copy h3 { color: var(--sand); font-family: 'Fraunces', serif; font-size: 1.9rem; margin-bottom: 18px; }
  ...
  @media (max-width: 480px) {
    ...
    .events-gallery img { height: 140px; }
    .capacity-grid { grid-template-columns: repeat(2,1fr); gap: 16px; }
    .max-cap { flex-direction: column; gap: 20px; }
    .venue-row { grid-template-columns: 1.2fr 0.8fr 1fr; font-size: 0.8rem; }
  }
</style>
