...
<section class="kids">
  <div class="kids-grid">
    <div class="kids-gallery" use:reveal>
    ...
    <div class="kids-copy" use:reveal={{ delay: 200 }}>
      <span class="eyebrow">{$t.kids.eyebrow}</span>
      <h3>{$t.kids.title}</h3>
      <p>{$t.kids.description}</p>
      <span class="note">{$t.kids.note}</span>
    </div>
  </div>
</section>
...
<style>
  :root {
    --green-deep: #16332A; --gold: #B8934A; --sand: #F1EAD9;
  ...
  .eyebrow { font-family: 'Work Sans', sans-serif; font-size: 0.72rem; letter-spacing: 0.22em; text-transform: uppercase; font-weight: 600; color: var(--gold); display: block; margin-bottom: 18px; }
  .kids-copy h3 { font-family: 'Fraunces', serif; font-size: 1.9rem; margin-bottom: 14px; color: var(--ink); }
  ...
  @media (max-width: 480px) {
    .kids-gallery { grid-template-columns: 1fr 1fr 1fr; }
    .kids-gallery img { height: 120px; }
  }
</style>
