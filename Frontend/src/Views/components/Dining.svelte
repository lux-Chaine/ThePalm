...
<section class="dining" id="dining">
  <div class="section-head" use:reveal>
    <span class="eyebrow">{$t.dining.eyebrow}</span>
    <h2>{$t.dining.title}</h2>
  </div>

  <div class="dining-gallery" use:reveal={{ delay: 100 }}>
  ...
  <div class="outlet-grid">
    {#each $t.dining.outlets as outlet, i}
      <div class="outlet-card" use:reveal={{ delay: (i + 2) * 100 }}>
        <h3>{outlet.name}</h3>
        <p>{outlet.description}</p>
        <dl class="outlet-meta">
          <div><dt>{$t.dining.metaLabels.cuisine}</dt><dd>{outlet.cuisine}</dd></div>
          <div><dt>{$t.dining.metaLabels.hours}</dt><dd>{outlet.hours}</dd></div>
          {#if outlet.breakfast}
            <div><dt>{$t.dining.metaLabels.breakfast}</dt><dd>{outlet.breakfast}</dd></div>
          {/if}
          {#if outlet.seating}
            <div><dt>{$t.dining.metaLabels.seating}</dt><dd>{outlet.seating}</dd></div>
          {/if}
          <div><dt>{$t.dining.metaLabels.location}</dt><dd>{outlet.location}</dd></div>
        </dl>
      </div>
    {/each}
  </div>
...
<style>
  :root {
    ...
    --gold-light: #D8BD86; --sand: #F1EAD9; --sand-warm: #EAE1CB;
    --paper: #F8F4EA; --ink: #201E19; --ink-soft: #4A493F;
  }
  ...
  .outlet-card h3 { font-family: 'Fraunces', serif; font-size: 1.3rem; margin-bottom: 12px; color: var(--ink); }
  ...
  .outlet-meta dd { font-family: 'Fraunces', serif; font-size: 0.95rem; color: var(--ink); }
  ...
  @media (max-width: 480px) {
    .dining-gallery { grid-template-columns: 1fr 1fr; gap: 10px; }
    .dining-gallery img { height: 160px; }
    .outlet-meta { grid-template-columns: 1fr; }
  }
</style>
