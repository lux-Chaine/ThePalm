...
-- IP-based throttling for the admin login form and the public
-- booking form. This is a second, independent layer on top of the
-- session-based limits already in the code — a session-only limit
-- can be bypassed just by clearing cookies, so attempts are also
-- tracked here by IP address.
-- ---------------------------------------------------------
CREATE TABLE IF NOT EXISTS `rate_limit_log` (
  `id`          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `bucket`      VARCHAR(40) NOT NULL,   -- e.g. 'login', 'booking'
  `ip_address`  VARCHAR(45) NOT NULL,   -- IPv4 or IPv6
  `created_at`  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX `idx_bucket_ip_time` (`bucket`, `ip_address`, `created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
