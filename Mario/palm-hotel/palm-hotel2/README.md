...
This package was reviewed end-to-end (code read-through + actually
running the site against a live MySQL database). Everything below
was already solid: parameterized queries throughout, CSRF tokens on
every state-changing form, bcrypt password hashing, escaped output,
and a self-locking setup wizard. Two real gaps were found and fixed:

1. **`config/`, `includes/`, and `sql/` had no proper `.htaccess`.**
   The original `.htaccess` only blocked specific file *extensions*
   (`.sql`, `.md`, `.log`, `.env`) — it did **not** block direct
   requests to `.php` files like `config/config.php`, which contains
   your database password. Each of those three folders (plus
   `admin/includes/`) now has its own `.htaccess` that denies all
   direct access outright. **Make sure these four `.htaccess` files
   are actually uploaded** — some FTP clients/hosts hide dotfiles by
   default, so double-check they exist in `public_html` after upload.

2. **Login and booking rate limits were session-only**, so anyone
   could bypass the "5 attempts per 10 minutes" style limits just by
   clearing cookies or using a private window. A second, independent
   layer was added: attempts are now also logged by IP address in a
   new `rate_limit_log` table (added to `sql/schema.sql`) and checked
   alongside the existing session-based limit.

Minor cleanups: removed a leftover unused line in `admin/room-form.php`,
corrected a misleading comment about `APP_SECRET` in `config.php`
(it isn't currently used for signing anything — CSRF protection relies
on per-session random tokens instead), and modernized the root
`.htaccess` for Apache 2.4 plus added basic security headers
(`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`).

If you later add an `/uploads/` folder for room photos (see the
Rooms section above), copy `uploads.htaccess.example` into it and
rename it to `.htaccess` — it stops any uploaded file from ever being
executed as a script, even if someone manages to upload something
malicious disguised as an image.
