...
/** Best-effort client IP (works behind Bluehost's normal setup; no proxy chain trust). */
...
/**
 * IP-based rate limit check, backed by the database so it can't be
 * bypassed by clearing cookies (unlike a session-only counter).
 * Returns true if the current IP has already hit the limit for this
 * bucket within the given time window.
 */
function rate_limit_hit(string $bucket, int $maxAttempts, int $windowSeconds): bool
{
    $stmt = db()->prepare(
        'SELECT COUNT(*) FROM rate_limit_log
         WHERE bucket = :bucket AND ip_address = :ip AND created_at > :since'
    );
    $stmt->execute([
        'bucket' => $bucket,
        'ip'     => client_ip(),
...
/** Record one attempt for the current IP in the given bucket. */
function rate_limit_record(string $bucket): void
{
    $stmt = db()->prepare('INSERT INTO rate_limit_log (bucket, ip_address) VALUES (:bucket, :ip)');
    $stmt->execute(['bucket' => $bucket, 'ip' => client_ip()]);

    // Occasionally prune old rows so the table doesn't grow forever
    // (no cron needed on shared hosting; ~1% chance per call).
    if (random_int(1, 100) === 1) {
        db()->exec("DELETE FROM rate_limit_log WHERE created_at < DATE_SUB(NOW(), INTERVAL 1 DAY)");
    }
}
